...
<?php
	$is_my_media = true;
	if($TABLE==='u_headline' && empty(getSorC('is_carousel_headline')) === false){
		//権限チェック
		...
		$is_my_media = isMyMedia($sid, explode(',', getSorC('u_media')));
	}
?>

<tr class="blockds" style="border-top:1px dotted #ccc;">
<?php if($NUMBERINGOFF!=1){ ?>
<td class="numbering"><?php if($p[$i]["n"]!=1 && $is_my_media){ ?><a<?php if(getSorC("order")==1){ ?> href="javascript://" onclick="javascript:cn(<?=$p[$i]["n"]?>,1)"<?php } ?>><img src="/shared/cms/img/cmd_up<?php if(getSorC("order")!=1 || $is_my_media === false){ ?>_disabled<?php } ?>.gif" width="13" height="13" alt="一つ上へ入れ替える" ></a><?php }else{ ?><img src="/shared/cms/img/cmd_ups<?php if(getSorC("order")!=1 || $is_my_media === false){ ?>_disabled<?php } ?>.gif" width="13" height="13" alt="一つ上へ入れ替える" ><?php } ?><?php if($p[$i]["n"]!=$N && $is_my_media){ ?><a<?php if(getSorC("order")==1){ ?> href="javascript://" onclick="javascript:cn(<?=$p[$i]["n"]?>,0)"<?php } ?>><img src="/shared/cms/img/cmd_down<?php if(getSorC("order")!=1 || $is_my_media === false){ ?>_disabled<?php } ?>.gif" width="13" height="13" alt="一つ下へ入れ替える" ></a><?php }else{ ?><img src="/shared/cms/img/cmd_downs<?php if(getSorC("order")!=1 || $is_my_media === false){ ?>_disabled<?php } ?>.gif" width="13" height="13" alt="一つ下へ入れ替える" ><?php } ?>
<?php } ?>
<!--
  ...
  <?php if ($CURRENTDIRECTORY != "company_news") { ?>
    <td class="display"><?php
      if($CURRENTDIRECTORY == "photo"){
        ...
        if(getSorC("draft")==1 && $is_my_media){
          echo sprintf("<div class=\"dp\"><img src=\"/shared/cms/img/cmd_%sactive.gif\" width=\"13\" height=\"13\" class=\"flagswitch lang_%s\" id=\"e%s\"></div>",$p[$i]["flag".$LANG[$EI]]!=1?"dis":"",$LANG[$EI],$p[$i]["id"]);
        }else{
...
<td class="colname"><?php include $INCLUDEPATH."lib/".$CURRENTDIRECTORY."/fetchdata.php"; ?></td>
    ...
    <?php }else{?>
        <td><a<?php if(getSorC("edit")==1 && $is_my_media){ ?> href="./edit/?<?=$EDITDELETEINITIAL?>id=<?=$p[$i]["id"]?>&<?=$g->g_url()?><?php if($CURRENTDIRECTORY=="repo_e"){ ?>&types=<?=$p[$i]["types"]?><?php } ?>"<?php } ?> class="menu"><span class="edit<?php if(getSorC("edit")!=1 || $is_my_media === false){ ?>_disabled<?php } ?>">編集</span></a></td>
    <?php }?>
    ...
    <?php }else{?>
                <td><a<?php if(getSorC("delete")==1 && $is_my_media){ ?> href="./delete/?<?=$EDITDELETEINITIAL?>id=<?=$p[$i]["id"]?>&<?=$g->g_url()?><?php if($CURRENTDIRECTORY=="repo_e"){ ?>&types=<?=$p[$i]["types"]?><?php } ?>"<?php } ?> class="menu"><span class="delete<?php if(getSorC("delete")!=1 || $is_my_media === false){ ?>_disabled<?php } ?>">削除</span></a></td>
    <?php }?>
</tr>
<?php } ?>
<?php }else{ ?>
