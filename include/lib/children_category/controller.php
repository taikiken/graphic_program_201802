...
$WHERE2 = " WHERE id IN (SELECT bottom_tab_id FROM bottom_tab_nodes WHERE parent_tab_id IS not NULL AND type=1 )";
$NUMBERINGOFF=1;
$type = 1;
...
CLD;
        ...
        //一覧の最後に追加
            ...
            $sv[$sn[]="sort_no"]=sprintf("(select max(sort_no)+1 as n from %s %s)",$TABLE,$WHERE2);
        }
