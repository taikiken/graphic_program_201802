...
<?php if($gBILL!=1){ ?>
<tr>
<td><?php if(strlen($DIRECTORY)>0){ ?><a href="<?=rewrite($DIRECTORY,$p[$i])?>" target="_blank"><img src="/shared/cms/img/icon_browser.gif" width="16" height="16" ></a><?php } ?>
<?php
...
if($_GET["cid"]==1){
	...
	$sql=sprintf("select title from repo_n where id=%s",$p[$i]["d2"]);
	$o->query($sql);
	$f=$o->fetch_array();
	...
	$sql=sprintf("select '['||a1||'-'||a2||'-'||a3||' '||a4||':'||a5||'] ' ||(select name from pm_ where id=m1)||'：'||title||'('||(select title from repo_n as e where e.id=repo_n.d2)||')' as title from repo_n where id=%s",$p[$i]["d2"]);
	$o->query($sql);
	$f=$o->fetch_array();
...
<td><?php if(strlen($p[$i]["m2"])>0){ ?><img src="/shared/img/common/ct<?=$p[$i]["m2"]?>.png" style="float:left;"><?php } ?><?php if(strlen($DIRECTORY)>0&&$TITLEFLAG==1){ ?><a href="<?=rewrite($DIRECTORY,$p[$i],$LANG[$EI])?>" target="_blank"><img src="/shared/cms/img/icon_browser.gif" width="16" height="16"></a><?php } ?><?=$TITLEFLAG==1?sprintf("[<b class=\"sedx\">%s</b>] ",timemapping($p[$i]["m_time"])).mod_HTML($p[$i]["title".$LANG[$EI]],1):"<span class=\"colred\">タイトル未設定</span>"?></td>
