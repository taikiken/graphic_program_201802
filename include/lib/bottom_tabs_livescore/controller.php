...
if($q->get_dir()===0){
    ...
    if($q->get_file()===0){
        ...
        $o->query($sql);
        $n=$o->fetch_array();
        $sql= "select count(DISTINCT bottom_tab_id) as c from bottom_tab_nodes where parent_tab_id = ".$n['id']." and type=2 and bottom_tab_id in (select id from bottom_tab_livescores) ";
        $o->query($sql);
        $count=$o->fetch_array();
        if ($count['c'] == 0){
            $sql=sprintf("update %s set sort_no=sort_no-1 where sort_no>=%s and id IN (SELECT bottom_tab_id FROM bottom_tab_nodes WHERE parent_tab_id IS NULL AND type=2 ) ",$TABLE,$n["sort_no"]);
            $o->query($sql);
            $o=new dbutl($TABLE);
            $e=$o->remove($g->f("id"));
