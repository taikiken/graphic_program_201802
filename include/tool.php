...
function makeTextfieldCell($a,$b,$c,$d,$e){
	...
	if($e==0){
		...
		for($i=0;$i<count($LANG);$i++){
			...
			if(!preg_match("/^(m|d|t40)/",$a)){
				echo sprintf("<span class=\"btncopy rollover %s\" id=\"%s\">コピー</span>",$LANG[$i],$c[0]);
			}else if(preg_match("/^(t40)/",$a)){
				echo sprintf("<span class=\"btncurrenttime rollover %s\" id=\"%s\">現在の日時に変更</span><br class=\"cboth %s\">",$LANG[$i],$c[0],$LANG[$i]);	
			}
		}
	}
...
function makeTextAreaCell($a,$b,$c,$d,$e,$f){
	global $q,$p,$f_name;
	echo sprintf("<tr><td%s class=\"inputTitle\">%s</td><td class=\"inputFields\">",strlen($e)>0?" rowspan=\"2\"":"",$a);
	if($f==0){
...
function makeTextAreaConf($a,$b,$c,$d,$e){
	...
	global $LANG;
	echo sprintf("<tr><td class=\"confTitle\">%s</td><td class=\"confFields\">",$a);
	if($e==0){
		$f=$d[$c.$b];
		echo strlen($f)>0?nl2br($f):"-";
	}else{
		for($i=0;$i<count($LANG);$i++){
			$f=$d[$c.$b.$LANG[$i]];
