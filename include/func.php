...
function sqlinj($q){
	...
	if(!strpos($CURRENTPATH,$ADPATH)){
		return str_replace(array("select","delete","update"),array("ｓｅｌｅｃｔ","ｄｅｌｅｔｅ","ｕｐｄａｔｅ"),$q);
	}else{
		return $q;
	}
}
...
function echo_hidden($f,$q=1){
	while(list($k,$v)=each($f)){
		if($q==1){
			...
			if(!(int)$k){
				if(!is_array($v)){
					echo "<input type=\"hidden\" name=\"".$k."\" value=\"".htmlspecialchars(str_replace(array("'","null"),"",$v))."\">\n";
				}else{
					for($j=0;$j<count($v);$j++){
						echo "<input type=\"hidden\" name=\"".$k."[]\" value=\"".htmlspecialchars(str_replace(array("'","null"),"",$v[$j]))."\">\n";
					}
				}
			}
		}
	...
	while(list($k,$v)=each($_POST)){
		if(ereg($q,$k)){
			if(strlen($v)>0){
				...
				$v=stripslashes($v);
				$v=addslashes($v);
				if(preg_match('/(m[0-9]+|d[0-9]+|fn[0-9]+)/',$k)){
...
function sendmail($to,$subject,$body,$from,$reply,$Bcc="",$f=""){
	...
	$msg=addslashes($msg);
	$msg=strs_split($msg,70);
	$msg=mb_convert_encoding($msg,"JIS","UTF-8");
	$header="From:=?iso-2022-jp?B?".base64_encode(mb_convert_encoding($SITE." WEBサイト 事務局","JIS","UTF-8"))."?=<".$from.">\n";
	...
	if(strlen($f)>0){
		$f=mb_convert_encoding($f,"UTF-8","UTF-8");
		...
		$body.=$msg."\n";
		$body.="--$boundary\n";
		$body.="Content-Type:application/octet-stream;name=\"".date("YmdHis")."csv\"\n";
		$body.="Content-Transfer-Encoding:base64\n";
		$body.="Content-Disposition:attachment;filename=\"".date("YmdHis").".csv\"\n\n";
		$body.=chunk_split(base64_encode($f))."\n\n";
		$body.="--".$boundary."\n";
	}else{
