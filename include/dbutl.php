...
class dbutl extends db{
	...
	function insert(){
		...
		if($this->table=="repo_n"&&$_GET["cid"]==1){
			...
			$this->sql.=sprintf("insert into repo_body(pid,body) values(currval('repo_n_id_seq'),%s);\n",strlen($body)>0?$body:"NULL");
		...
		if($this->table=="repo_n"&&$_GET["cid"]==1){
			...
			for($i=0;$i<count($this->columns);++$i){
				...
				if(preg_match("/^(b|t)[2-6]{1}$/",$this->columns[$i])){
					...
					$body=str_replace("<br />","<br>",$body);
					$body=preg_replace("/style=\".*?\"|style='.*?'/i","",$body);
					if(strlen($body)>0)$this->sql=sprintf("update repo_body set body=%s where pid=%s;\n",$body,$t);
				}else{
