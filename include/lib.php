...
elseif ($TABLE == "tbl_player")
{
	...
	$sql=sprintf("select %s from %s%s order by always_update_flag desc, n%s %s",$FIELD,$TABLE,$WHERE,($CURRENTDIRECTORY=="log"||preg_match("#/photo/#",$_SERVER["REQUEST_URI"]))?" desc":"",dblm($no,$offset));
}else{
