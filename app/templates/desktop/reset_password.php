...
if(isset($_POST["setting-form-mail"])){
	...
	function mailreminder($email,$name,$hash){
		...
		http://dev.undotsushin.com/reset_password/resetting/?m=%s
		
		ご不明な点等ございましたら、以下の運動通信カスタマーセンターまでお問い合わせください。
		info@undotsushin.com
		
		[ご注意]
		こちらのメールアドレスは送信専用のため、直接返信されても返答できませんので予めご了承ください。",$name,$hash);
...
<?php 
	...
	include str_replace("public","",$_SERVER["DOCUMENT_ROOT"])."/app/templates/desktop/reset_password.complete.php";
}
?>