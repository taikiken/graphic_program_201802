...
$sql="select id,img1,title,a2,a3 from repo_n where d2=54 and flag=1 and swf is not null order by (a1||'-'||a2||'-'||a3||' '||a4||':'||a5||':'||a6)::timestamp desc limit 4 offset 0";
$db->query($sql);
...
$sql="select id,img1,title,a1,a2,a3 from repo_n where d2=54 and flag=1 and swf is null order by (a1||'-'||a2||'-'||a3||' '||a4||':'||a5||':'||a6)::timestamp desc limit 6 offset 0";
$db->query($sql);
while($f=$db->fetch_array()){
	$v=array(
		"title"=>htmlspecialchars($f["title"]),
