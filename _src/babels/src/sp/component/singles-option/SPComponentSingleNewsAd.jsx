 ...
 * @since 2016-09-21
 */
export default class SPComponentSingleNewsAd extends React.Component {
  // ---------------------------------------------------
  ...
  /**
   * プロパティと初期変数を設定します
   * @param {Object} props React.props {@link SPComponentSingleNewsAd.propTypes}
   */
  constructor(props) {
    super(props);
     ...
     */
    this.state = {
      // index が 3の倍数 の時に広告をinsert
      // 2018-01-18 - indexが2の時に広告を出すから改修
      // third: props.index === 2
      multiple_3: ((props.index + 1) % 3) === 0
    };
    /**
     * 広告フラッグ（挿入済み）
     * @type {boolean}
  ...
  ad() {
    ...
    return (
      <div
        className={`board-item news-ad news-ad-${this.props.index} board-item-ad-wrapper`}
  ...
  componentDidMount() {
    ...
    if (this.newsAd) {
      // console.log('SPComponentSingleNewsAd.componentDidMount', this.props.adSp);
  ...
  render() {
    ...
    // console.log('SPComponentSingleNewsAd.render', big6tv, this.props);
    if (big6tv) {
      return null;
    }
    // ストリーム広告
