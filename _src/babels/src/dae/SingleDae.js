...
export class SingleDae extends RelatedDae {
  ...
  constructor(response = {}) {
    ...
    //  * @protected
    //  */
    // this._keywords = new KeywordsDae( altResponse.keywords );
    // // console.log( 'SingleDae _keywords' );
    // // related
    // let related = [];
    // if ( Safety.check( altResponse, 'related_articles', 'array' ) ) {
    //   //
    ...
    //   //
    //   // } );
    //   related = altResponse.related_articles.map((article) => new RelatedDae(article));
    // }
    // /**
    ...
    //  * @private
    //  */
    // this._canonical = new CanonicalDae(altResponse.canonical);
    //
    // const recommendArticles = Safety.array(altResponse.recommend_articles);
    // /**
    ...
    //  * @private
    //  */
    // this._readmore = new ReadmoreDae(altResponse.readmore);
    // ----
    /**
     * キーワード - `response.keywords`
     * @type {KeywordsDae}
     */
    this.keywords = new KeywordsDae(altResponse.keywords);
    const related = Safety.check(altResponse, 'related_articles', 'array') ?
      altResponse.related_articles.map(article => (new RelatedDae(article))) :
      [];
    /**
     * 関連記事 list - `response.related_articles`
     * @type {Array.<RelatedDae>}
     */
    ...
    /**
     * カノニカル - `response.canonical`
     * @type {CanonicalDae}
     */
    this.canonical = new CanonicalDae(altResponse.canonical);
    const recommendArticles = Safety.array(altResponse.recommend_articles);
    /**
     ...
     * @type {Array.<SingleDae>}
     */
    this.recommendArticles = recommendArticles.map(article => (new SingleDae(article)));
    /**
     * 「続きを読む」フラッグ - `response.readmore`
     * @type {ReadmoreDae}
     */
    this.readmore = new ReadmoreDae(altResponse.readmore);
    /**
     * 記事本文 - `response.body`
     * @type {string}
     */
    this.body = altResponse.body;
    /**
     * 記事本文(htmlなし） - `response.body_escape`
     * @type {string}
     * @since 2016-09-25
     */
    this.bodyEscape = altResponse.body_escape || '';
    /**
     * 関連記事が存在するかの flag 真偽値
     * @type {boolean}
     */
    this.hasRelated = related.length > 0;
    /**
     * [記事詳細 / 任意の記事詳細での画像非表示( e-player埋め込み対応 )](https://github.com/undotsushin/undotsushin/issues/721) - `response.is_show_image`
     *
     * 記事詳細で画像を表示するかどうか
     * - true  : 表示する
     * - false : 表示しない
     ...
     * @since 2016-06-06
     */
    this.isShowImage = altResponse.is_show_image;
    /**
     ...
     * @since 2016-09-25
     */
    this.mediaVkRefid = altResponse.media_vk_refid;
    /**
     * response.information - カテゴリー情報APIと同様の以下を出力(設定内容はCMS上の本記事のプライマリーカテゴリー引き継ぎ）
     * @type {InformationDae}
