...
import { CommentsPopularDae } from '../../dae/CommentsPopularDae';
...
/**
 * 記事一覧・コメント・一段目
 * @param {CommentsPopularDae} commentsPopular JSON comment popular
 * @param {number} totalNum 件数
 * @param {string} articleId 記事 ID
 * @param {string} uniqueId unique ID
 * @return {?XML} `div.comments-popular`
 * @constructor
 * @since 2016-09-16
 * @since 2017-12-22 - update stateless React component
 */
const ComponentArticlePopular = ({ commentsPopular, totalNum, articleId, uniqueId }) => {
  const hasFirst = commentsPopular.hasFirst;
...
/**
 * React.propTypes
 * @type {{commentsPopular: CommentsPopularDae, total: number, articleId: string, uniqueId: string}}
 */
ComponentArticlePopular.propTypes = {
  commentsPopular: React.PropTypes.instanceOf(CommentsPopularDae).isRequired,
  total: React.PropTypes.number.isRequired,
  articleId: React.PropTypes.string.isRequired,
  uniqueId: React.PropTypes.string.isRequired,
};

export default ComponentArticlePopular;
