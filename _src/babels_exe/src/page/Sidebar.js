...
export default class Sidebar {
   ...
   */
  static start(slug = 'all', home = false) {
    // ranking
    ...
    if (rankingElement !== null) {
      ...
      const ranking = new UT.view.sidebar.ViewRanking(rankingElement, option, slug);
      ranking.start();
    }
    ...
    if (videoElement !== null) {
      ...
      const videos = new UT.view.sidebar.ViewVideos(videoElement, option, slug);
      videos.start();
    }
    ...
    if (recommendElement !== null) {
      const option = {};
      const recommend = new UT.view.sidebar.ViewRecommend(recommendElement, option, slug);
      recommend.start();
    }
    // sidebar
   ...
   */
  static prepareMoving(home) {
    // git branch で分岐しすぎていて関数がなくエラーにならないために存在チェックを行います
    const sidebar = !!Dom.sidebar ? Dom.sidebar() : null;
    const footer = !!Dom.footer ? Dom.footer() : null;
   ...
   */
  static moving(sidebar, footer, home) {
    ...
    // optimize 2-17-12-04
    // home のみ pickup element が存在する
    const pickup = home ? Dom.pickup() : null;

    const move = new UT.view.sidebar.Sidebar(sidebar, footer);
   ...
   */
  static rankingBeforeRender(articles) {
    // ranking
    const rankingElement = Dom.ranking2();
    if (rankingElement !== null) {
      const option = {};
      option[UT.view.View.DID_MOUNT] = Sidebar.didRanking2;
      const ranking = new UT.view.sidebar.ViewRanking(rankingElement, option);
   ...
   */
  static videosBeforeRender(articles) {
    // video
    const videoElement = Dom.video2();

    if (videoElement !== null) {
      const option = {};
      option[UT.view.View.DID_MOUNT] = Sidebar.didVideo2;
      const videos = new UT.view.sidebar.ViewVideos(videoElement, option);
