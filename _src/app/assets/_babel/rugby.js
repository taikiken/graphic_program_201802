...
const ranking = () => {
  ...
  const viewRanking = new ViewRanking(rankingElement, option, 'rugby');
  viewRanking.start();
};
...
const video = () => {
  ...
  option[View.DID_MOUNT] = didVideo;
  const viewVideos = new ViewVideos(videoElement, option, 'rugby');
  viewVideos.start();
};
...
const recommend = () => {
  ...
  const viewRecommend = new ViewRecommend(recommendElement, {}, 'rugby');
  viewRecommend.start();
};
