...
const ranking = () => {
  ...
  const viewRanking = new ViewRanking(rankingElement, option, 'big6tv');
  viewRanking.start();
};
...
const video = () => {
  ...
  option[View.DID_MOUNT] = didVideo;
  const viewVideos = new ViewVideos(videoElement, option, 'big6tv');
  viewVideos.start();
};
...
const recommend = () => {
  ...
  const viewRecommend = new ViewRecommend(recommendElement, {}, 'big6tv');
  viewRecommend.start();
};
