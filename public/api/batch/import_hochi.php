...
function get_index()
{
	...
	$dailyVal = [
		"http://spb_hochi:e8fqLq2_hc@feed.yomiuri.co.jp/rss/hochisportsbull/hochi_sportsbull.xml"
	];
	$u = [];
...
for($i=0;$i<count($data);$i++)
{
	...
	if ($s["m1"] == 129)
	{
		//もし「その他競技」にマッピングされてしまうならば、カテゴリから検索
		$o->query(sprintf("select * from u_categories  WHERE \"name\" = '%s'", $data[$i]["category"]));

		$category=$o->fetch_array();

		if ($category !== false){
			$s["m1"] = (int)$category["id"];
		}
	}
	...
	if(strlen($f["id"])>0){
		...
		$s["flag"]=1;
		$s["cid"]=1;
		if(preg_match("/全国高等学校野球選手権大会/",$modbody)){
			$s["m1"]=136;
		}
