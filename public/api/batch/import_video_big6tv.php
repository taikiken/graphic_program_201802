...
$dev=array("title","a1","a2","a3","a4","a5","a6","a7","a8","a9","a10","a11","a12","u_time","img1","t1","t2","t3");
$cms=array("title","a1","a2","a3","a4","a5","a6","a7","a8","a9","a10","a11","a12","u_time","img1","swf","t10","t11");
...
while(list($k,$v)=each($data)){
	...
	if($v["flag"]!=1&&$dbdata["flag"]==1)$sv["flag"]=0;
	...
	if(!isset($dbdata["id"])&&$v["uid"]==0){
		...
		if(strtotime($v["u_time"])>strtotime($dbdata["u_time"])){
			...
			while(list($kk,$vv)=each($sv)){
				if(!preg_match("/flag/",$kk)&&$dbdata[$kk]!=$vv)$qq[]=sprintf("%s='%s'",$kk,bind($vv));
			}
	...
	if($e){
		...
		if($announce==1){
			get_contents(sprintf("https://sportsbull.jp/api/batch/sendmail.php?id=%s&file=%s&t2=%s&t3=%s",$id,$sv["swf"],urlencode($sv["t10"]),urlencode($sv["t11"])));
