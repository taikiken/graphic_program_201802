		...
		if(strlen($uid)>0){
			...
			if($status["flag"]==0||$status["reaction"]!=$type||strlen($status["reaction"])==0){
				...
				if(strlen($status["reaction"])==0){
					$sql =sprintf("insert into u_reaction(commentid,userid,reaction,flag,regitime) values(%s,%s,%s,1,now());\n",$commentid,$uid,$type);
					...
					$sql=sprintf("update u_reaction set reaction=%s,flag=1,regitime=now() where id=%s;\n",$type,$f["id"]);
