...
if($y["status"]["code"]===200){
	...
	select *,(select title from repo_n where id=pageid and flag=1) as title,(select flag from u_member where id=userid) as uflag from u_activity where reuserid=%s and flag=1 and activity!=4 and activityid is not null order by id desc limit %s offset %s) as t1
	where t2.uid=t1.reuserid order by t1.id desc;",$uid,$uid,$length,$offset);

	$nsql=sprintf("
	select count(t1.reuserid) as n from 
	(select id as uid from u_member where id=%s and flag=1) as t2,
	(select reuserid,(select title from repo_n where id=pageid and flag=1) as title,(select flag from u_member where id=reuserid) as uflag from u_activity where reuserid=%s and flag=1 and activity!=4 and activityid is not null) as t1
