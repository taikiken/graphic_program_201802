...
/*
	...
	select id,userid,reuserid,pageid,activity,activityid,notice,flag,regitime,(select title from repo_n where id=pageid and flag=1) as title from u_activity where activity=1 and userid=%s and flag=1 and activityid is not null and (select flag from u_member where id=reuserid)=1 union all 
	select id,userid,reuserid,pageid,activity,activityid,notice,flag,regitime,(select title from repo_n where id=pageid and flag=1) as title from u_activity where activity in(2,3) and userid=%s and flag=1 and activityid is not null and (select flag from u_member where id=reuserid)=1 union all 
	select id,userid,reuserid,pageid,activity,activityid,notice,flag,regitime,(select title from repo_n where id=pageid and flag=1) as title from u_activity where activity=4 and userid=%s and flag=1 and activityid is not null and (select flag from u_member where id=reuserid)=1 ) as t1
	where t2.uid=t1.userid order by regitime desc limit %s offset %s
*/
	...
	$fls=array("id","commentid","userid","comment","regitime","pid","typeid","type","name","profile","icon");
	...
	for($i=0;$i<count($p);$i++){
		...
		if($p[$i]["activity"]!=4){
			...
			$sql=sprintf("select %s,pageid from (select * from (select id,flag,commentid,userid,pageid,comment,regitime from u_comment where id=%s) as t1,(select id as pid,cid as typeid,(select name from repo where id=cid) as type,title as name,t2 as profile,img1 as icon from u_member where qid=2 and flag=1) as t2 where t1.userid=t2.pid) as c1 left join (select * from (select id,flag,commentid,userid,comment,regitime from u_comment) as t1,(select id as pid,cid as typeid,(select name from repo where id=cid) as type,title as name,t2 as profile,img1 as icon from u_member where qid=2 and flag=1) as t2 where t1.userid=t2.pid) as c2 on c1.commentid=c2.id",$e,
