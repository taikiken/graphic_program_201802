...
if(preg_match('/^[0-9]+$/',$id)){
	...
	if(strlen($f["id"])>0){
		...
		while($f=$o->fetch_array()){
			...
			$s["related_articles"][$n]["categories"][0]["label"]=$f["category"]; 
			$s["related_articles"][$n]["categories"][0]["slug"]=$f["slug"];
			if(strlen($f["category2"])>0){
				$s["related_articles"][$n]["categories"][1]["label"]=$f["category2"]; 
				$s["related_articles"][$n]["categories"][1]["slug"]=$f["slug2"]; 
			}
	...
	$y["status"]["code"]=400;
	$y["status"]["user_message"]="指定されたページは存在しません。";
	$y["status"]["developer_message"]="IDに不正値がある";
	$s=(object)array();
}
