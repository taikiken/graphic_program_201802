...
if(strlen($uid)>0){
	...
	$tmptable=sprintf("select t1.id from (select id,activityid from u_activity where activity=1 and reuserid=%s and flag=1) as t1,(select id from u_comment where flag=1) as t2 where t1.activityid=t2.id union select t1.id from (select id,activityid from u_activity where activity in(2,3) and reuserid=%s and flag=1) as t1,(select id from u_reaction where flag=1) as t2 where t1.activityid=t2.id",$uid,$uid);
	...
	$nsql=sprintf("select count(*) as n from (select userid from u_activity where id in (%s)) as c1,(select id from repo_n where qid=2 and flag=1) as c2 where c1.userid=c2.id",$tmptable);
	...
	$y["status"]["developer_message"]="Tokenが未設定。";	
}
