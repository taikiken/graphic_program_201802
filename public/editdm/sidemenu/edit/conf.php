<?php
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || isset($_POST['sidemenu']) === "false") {
...
//json形式の確認
$d = json_decode($sidemenu_text,true);
$notJson = false;
if(empty($d))$notJson = true;
        ...
        <form name="f" action="./exe.php" method="post">
            <div id="mainArea">
                <div id="pageDescription" class="clearfix">
                    <p>新規sidemenu.txtを登録します。</p>
                    <?php if($notJson){?>
                    <br><p style="color:red;">入力内容がjson形式ではありません。</p>
                    <?php } ?>
                </div>
                <!-- End pageDescription -->
                <table border="0" cellspacing="0" cellpadding="0" summary="sidemenu.text設定項目" class="listTable">
                    <tbody>
                        <tr class="bottomborder">
                            <td class="inputFields"><?php echo nl2br(htmlspecialchars($sidemenu_text)); ?></td>
                            <textarea class="hidetd" name="sidemenu"><?php echo htmlspecialchars($sidemenu_text); ?></textarea>
                        </tr>
                    </tbody>
                </table>
                <div class="debug">
